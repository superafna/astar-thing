...
def grid_gen(row=randint(50, 100), col=randint(50, 100)) -> np.ndarray:
    ...
    """
    grid = np.stack([[max(randint(0, 1), randint(0, 1)) for _ in range(col)] for _ in range(row)])
    return grid
...
def point_gen_new(row:int, col:int) -> tuple[int, int]:
    """
    Randomly generate a point within a 2d grid.
    :param row: len of row.
    :param col: len of grid.
    :return: coordinates as tuple of ints.
    """
